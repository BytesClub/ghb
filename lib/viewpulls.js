...
var viewPulls = (pullFd) => {
	if (pullFd.trim() === ''){
		let err = `No Git pulls found! You may fetch again for latest updates.`
		throw err;
	}
	let pulls = JSON.parse(pullFd)
	if (typeof issues === 'undefined' || (Object.keys(issues).length === 0 && issues. constructor === Object)){
		let err = `Fatal Error: No Git pulls found in local data!\n\nUse ${argv[index]} fetch pulls to get the latest.`
		throw err;
	}
...
// Usage: this.viewPulls('ght.pulls.json')