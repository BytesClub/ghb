...
class GitHubT {
	constructor(props) {
		if (props.type === "json") {
			try {
				const fPath = props.data.dir + "/" + props.data.file;
				let fData, fIndex;
				while(true) {
					try {
						fData = fs.readFileSync(fPath, "utf8");
						if (typeof fData !== "undefined")
							break;
					} catch(err) {
						if (err.code === "ENOENT" &&
                            (fIndex = fPath.lastIndexOf("-")) !== -1) {

                            fPath = fPath.substr(0, fIndex);
						} else {
							throw err;
						}
					}
				}
				props.data.file  = fPath.substr(fPath.indexOf("/") + 1);
				const json       = JSON.parse(fData);
				this._data       = {};
				this._data.url   = json.url;
				this._data.issue = new Issue({
                    url: this._data.url,
                    data: json.issue.issues,
                    path: props.data
                });
				this._data.pull  = new Pulls({
                    url: this._data.url,
                    data: json.pull.pulls,
                    path: props.data
                });
			} catch(e) {
				const err = "Invalid Error Occured: Make sure GitHub Terminal is initiated correctly.";
				throw err;
			}
		}
		else if (props.type === "url" && (typeof props.data === "string" &&
            props.data.trim().length !== 0 && URL.test(props.data))) {
			...
			try {
				...
				const fPath = props.path.dir + "/" + props.path.file;
	...
	get get_url() {
		if (typeof this._data === "undefined" || typeof this._data.url === "undefined") {
			let err = "GitHub Terminal has not been initiated " +
                "in this repository.";
			throw err;
		}
